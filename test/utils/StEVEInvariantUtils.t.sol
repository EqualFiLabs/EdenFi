...
contract StEVEInvariantInspector {
    struct PoolSnapshot {
        address underlying;
        bool initialized;
...
contract StEVEInvariantHandler {
    ...
    struct HandlerConfig {
        ...
        StEVEInvariantInspector inspector;
        MockERC20Launch eve;
        MockERC20Launch alt;
    ...
    StEVEInvariantInspector public immutable inspector;
    ...
    constructor(HandlerConfig memory cfg, address[] memory actors_) {
        ...
        feeBasketPoolId = EqualIndexAdminFacetV3(cfg.diamond).getIndexPoolId(cfg.feeBasketId);
    ...
    function mintWalletFeeBasket(uint256 actorSeed, uint256 unitsSeed) external {
        ...
        EqualIndexActionsFacetV3(diamond).mint(feeBasketId, units, actor, maxInputs);
        vm.stopPrank();
        _syncRewardIndex();
    }
    ...
    function burnWalletFeeBasket(uint256 actorSeed, uint256 unitsSeed) external {
        ...
        vm.prank(actor);
        EqualIndexActionsFacetV3(diamond).burn(feeBasketId, units, actor);
        _syncRewardIndex();
    }
    ...
    function burnFeeBasketFromPosition(uint256 positionSeed, uint256 unitsSeed) external {
        ...
        address owner = positionNft.ownerOf(positionId);
        vm.prank(owner);
        EqualIndexPositionFacet(diamond).burnFromPosition(positionId, feeBasketId, units);
        _syncRewardIndex();
    }
    ...
    function mintWalletStEVE(uint256 actorSeed, uint256 unitsSeed) external {
        ...
        uint256[] memory maxInputs = new uint256[](1);
        maxInputs[0] = units;
        StEVEWalletFacet(diamond).mintStEVE(units, actor, maxInputs);
        vm.stopPrank();
        _syncRewardIndex();
    }
    ...
    function withdrawStEVEFromPosition(uint256 positionSeed, uint256 amountSeed) external {
        ...
        address owner = positionNft.ownerOf(positionId);
        vm.prank(owner);
        StEVEActionFacet(diamond).withdrawStEVEFromPosition(positionId, amount, 0);
        _syncRewardIndex();
    }
    ...
    function borrowAgainstAltBasket(
        ...
        uint256 depositAmount = _units(depositSeed, 25, 150);
        ...
        if (mintUnits < collateralUnits) {
            mintUnits = collateralUnits;
        }
        _mintWalletStEVEForOwner(positionId, mintUnits);
        _depositWalletStEVE(positionId, mintUnits);

        StEVELendingFacet.BorrowPreview memory preview =
        ...
        address owner = positionNft.ownerOf(positionId);
        vm.prank(owner);
        uint256 loanId = StEVELendingFacet(diamond).borrow(positionId, preview.collateralUnits, duration);
    ...
    function repayLoan(uint256 loanSeed) external {
        ...
        StEVELendingFacet.RepayPreview memory preview = StEVELendingFacet(diamond).previewRepay(positionId, loanId);
    ...
    function extendLoan(uint256 loanSeed, uint256 durationSeed) external {
        ...
        StEVELendingFacet.ExtendPreview memory preview =
            StEVELendingFacet(diamond).previewExtend(positionId, loanId, extraDuration);

        address owner = positionNft.ownerOf(positionId);
        vm.prank(owner);
        StEVELendingFacet(diamond).extend{value: preview.feeNative}(positionId, loanId, extraDuration);
        _syncRewardIndex();
    }
    ...
    function attemptUnauthorizedMutations(uint256 actorSeed, uint256 actionSeed) external {
        ...
        if (actionSeed % 6 == 0) {
            vm.prank(actor);
            (ok,) = diamond.call(abi.encodeWithSelector(StEVEAdminFacet.setProtocolURI.selector, "ipfs://bad"));
        } else if (actionSeed % 6 == 1) {
            vm.prank(actor);
            (ok,) = diamond.call(abi.encodeWithSelector(StEVEAdminFacet.setPoolFeeShareBps.selector, uint16(1234)));
            ...
            (ok,) = address(timelockController)
                .call(
                    abi.encodeWithSelector(
                        ...
                        diamond,
                        uint256(0),
                        abi.encodeWithSelector(StEVEAdminFacet.setContractVersion.selector, "bad"),
                        bytes32(0),
                        keccak256("bad-salt"),
                        7 days
                    )
    ...
    function _mintAltBasketFromPositionInternal(uint256 positionId, uint256 units) internal {
        ...
        address owner = positionNft.ownerOf(positionId);
        vm.prank(owner);
        EqualIndexPositionFacet(diamond).mintFromPosition(positionId, altBasketId, units);
    ...
    function _mintFeeBasketFromPositionInternal(uint256 positionId, uint256 units) internal {
        ...
        address owner = positionNft.ownerOf(positionId);
        vm.prank(owner);
        EqualIndexPositionFacet(diamond).mintFromPosition(positionId, feeBasketId, units);
    ...
    function _mintWalletStEVEForOwner(uint256 positionId, uint256 amount) internal {
        address owner = positionNft.ownerOf(positionId);
        address underlying = StEVEViewFacet(diamond).getProductConfig().assets[0];
        if (underlying == address(eve)) {
            eve.mint(owner, amount);
            vm.startPrank(owner);
            eve.approve(diamond, amount);
        } else if (underlying == address(alt)) {
            alt.mint(owner, amount);
            vm.startPrank(owner);
            alt.approve(diamond, amount);
        } else {
        ...
        uint256[] memory maxInputs = new uint256[](1);
        maxInputs[0] = amount;
        StEVEWalletFacet(diamond).mintStEVE(amount, owner, maxInputs);
        vm.stopPrank();
        _syncRewardIndex();
    }
    ...
    function _depositWalletStEVE(uint256 positionId, uint256 amount) internal {
        ...
        StEVEActionFacet(diamond).depositStEVEToPosition(positionId, amount, amount);
        vm.stopPrank();
        _markJoined(positionId, stevePoolId);
        _syncRewardIndex();
    }
    ...
    function _quoteSingleAssetBasketMint(uint256 basketId, uint256 units) internal view returns (uint256 required) {
        EqualIndexBaseV3.IndexView memory basket = EqualIndexAdminFacetV3(diamond).getIndex(basketId);
        uint256 totalSupply = basket.totalUnits;
        uint256 vaultBalance = EqualIndexAdminFacetV3(diamond).getVaultBalance(basketId, basket.assets[0]);
        uint256 feePot = EqualIndexAdminFacetV3(diamond).getFeePot(basketId, basket.assets[0]);
    ...
    function _pickActiveLoan(uint256 seed) internal view returns (uint256 loanId, bool found) {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            StEVELendingFacet.LoanView memory loanView = StEVELendingFacet(diamond).getLoanView(candidate);
            if (loanView.active) {
