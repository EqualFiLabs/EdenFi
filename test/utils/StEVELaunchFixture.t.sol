...
abstract contract StEVELaunchFixture is DeployEqualFi {
    ...
    function _bootstrapStEVEProduct() internal {
        _bootstrapCorePools();
    ...
    function _createBasket(StEVEProductBase.CreateBasketParams memory params)
        internal
        pure
        returns (uint256 basketId, address basketToken)
    {
        ...
        revert("generic product basket creation removed");
    }

    function _createStEVE(StEVEProductBase.CreateBasketParams memory params)
        internal
        returns (uint256 basketId, address basketToken)
    {
        _timelockCall(diamond, abi.encodeWithSelector(StEVEActionFacet.createStEVE.selector, params));
    ...
    function _configureLending(uint40 minDuration, uint40 maxDuration) internal {
        _timelockCall(
            diamond, abi.encodeWithSelector(StEVELendingFacet.configureLending.selector, minDuration, maxDuration)
        );
    }
    ...
    function _configureBorrowFeeTiers(uint256[] memory minCollateralUnits, uint256[] memory flatFeeNative) internal {
        _timelockCall(
            diamond,
            abi.encodeWithSelector(StEVELendingFacet.configureBorrowFeeTiers.selector, minCollateralUnits, flatFeeNative)
        );
    }

    function _setProductMetadata(string memory uri, uint8 productType) internal {
        _timelockCall(diamond, abi.encodeWithSelector(StEVEAdminFacet.setProductMetadata.selector, uri, productType));
    }
    ...
    function _setProtocolURI(string memory uri) internal {
        _timelockCall(diamond, abi.encodeWithSelector(StEVEAdminFacet.setProtocolURI.selector, uri));
    }

    function _setContractVersion(string memory version_) internal {
        _timelockCall(diamond, abi.encodeWithSelector(StEVEAdminFacet.setContractVersion.selector, version_));
    }

    function _setFacetVersion(address facet, string memory version_) internal {
        _timelockCall(diamond, abi.encodeWithSelector(StEVEAdminFacet.setFacetVersion.selector, facet, version_));
    }

    function _setProductPaused(bool paused) internal {
        _timelockCall(diamond, abi.encodeWithSelector(StEVEAdminFacet.setProductPaused.selector, paused));
    }
    ...
    function _setProductFees(uint16[] memory mintFeeBps, uint16[] memory burnFeeBps, uint16 flashFeeBps) internal {
        _timelockCall(
            diamond,
            abi.encodeWithSelector(StEVEAdminFacet.setProductFees.selector, mintFeeBps, burnFeeBps, flashFeeBps)
        );
    }
    ...
    function _setPoolFeeShareBps(uint16 poolFeeShareBps) internal {
        _timelockCall(diamond, abi.encodeWithSelector(StEVEAdminFacet.setPoolFeeShareBps.selector, poolFeeShareBps));
    }

    function _setTimelockController(address newController) internal {
        _timelockCall(diamond, abi.encodeWithSelector(StEVEAdminFacet.setTimelockController.selector, newController));
    ...
    function _mintWalletBasket(address user, uint256 basketId, ERC20 asset, uint256 units) internal {
        ...
        StEVEWalletFacet(diamond).mintStEVE(units, user, maxInputs);
        vm.stopPrank();
    }
    ...
    function _singleAssetParams(
        ...
        uint16 mintFee,
        uint16 burnFee
    ) internal pure returns (StEVEProductBase.CreateBasketParams memory p) {
        p.name = name_;
        p.symbol = symbol_;
        p.uri = uri_;
    ...
    function _stEveParams(address asset) internal pure returns (StEVEProductBase.CreateBasketParams memory p) {
