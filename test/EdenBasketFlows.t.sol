...
contract EdenBasketFlowsTest is EdenLaunchFixture {
    ...
    function test_CreateBasket_RevertsForMissingCanonicalPool() public {
        ...
        bytes memory data = abi.encodeWithSelector(ILegacyEdenWalletFacet.createBasket.selector, params);
    ...
    function test_MintBasket_RevertsForInvalidUnitsPausedBasketAndInputShape() public {
        ...
        ILegacyEdenWalletFacet(diamond).mintBasket(basketId, 10e18, bob, wrongLen);

        uint256[] memory maxInputs = new uint256[](1);
        maxInputs[0] = 11e18;
        vm.expectRevert(InvalidUnits.selector);
        ILegacyEdenWalletFacet(diamond).mintBasket(basketId, 0, bob, maxInputs);
