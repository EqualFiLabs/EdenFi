...
**Finding 2 — Solo AMM swap missing ActiveCreditIndex update on encumbrance changes**

1.3 WHEN `_applyReserveDelta` increases `enc.encumberedCapital` during a swap THEN the system does not call `LibActiveCreditIndex.applyEncumbranceIncrease(...)`, causing `activeCreditPrincipalTotal` to drift from actual encumbrance

1.4 WHEN `_applyReserveDelta` decreases `enc.encumberedCapital` during a swap THEN the system does not call `LibActiveCreditIndex.applyEncumbranceDecrease(...)`, causing stale ACI weighting for the life of the market

**Finding 3 — Solo AMM `_closeMarket` conditional fee subtraction inflates `trackedBalance`**

1.5 WHEN `_closeMarket` subtracts protocol fees from `reserveForPrincipal` using per-fee conditional checks (`if feeAccrued > 0 && reserve >= feeAccrued`) and cumulative protocol fees on one side exceed the remaining reserve THEN the system skips the second fee subtraction, leaving protocol-fee amounts inside maker principal and inflating `trackedBalance`

**Finding 4 — Community AMM share inflation via join math**

1.6 WHEN a new maker joins a Community AMM market after swaps have grown reserves through retained fees THEN the system mints shares using `sqrt(amountA * amountB)` instead of a proportional formula, granting excess ownership relative to current pool state and diluting existing makers

**Finding 5 — Solo AMM cancel missing time guard**

1.7 WHEN the maker calls `cancelEqualXSoloAmmMarket` at or after `market.startTime` THEN the system allows cancellation of a live market, enabling the maker to pull liquidity after trading has opened
...
**Finding 2 — Live ACI synchronization**

2.3 WHEN `_applyReserveDelta` increases `enc.encumberedCapital` during a swap THEN the system SHALL call `LibActiveCreditIndex.applyEncumbranceIncrease(pool, poolId, makerPositionKey, delta)` so that `activeCreditPrincipalTotal` stays synchronized with encumbrance

2.4 WHEN `_applyReserveDelta` decreases `enc.encumberedCapital` during a swap THEN the system SHALL call `LibActiveCreditIndex.applyEncumbranceDecrease(pool, poolId, makerPositionKey, delta)` so that ACI weighting remains current

**Finding 3 — Deterministic close-time fee subtraction**

2.5 WHEN `_closeMarket` computes `reserveForPrincipal` THEN the system SHALL subtract total protocol fees per side (`feeIndexFeeAccrued + activeCreditFeeAccrued`) in a single operation and clamp to zero when cumulative fees exceed the remaining reserve, using `reserveForPrincipal = reserve > totalProtocol ? reserve - totalProtocol : 0`

**Finding 4 — Proportional share minting**

2.6 WHEN a maker joins a Community AMM market that already has `totalShares > 0` THEN the system SHALL mint shares using the proportional formula `share = min(amountA * totalShares / reserveA, amountB * totalShares / reserveB)` instead of `sqrt(amountA * amountB)`

**Finding 5 — Cancel time guard**

2.7 WHEN the maker calls `cancelEqualXSoloAmmMarket` at or after `market.startTime` THEN the system SHALL revert, preventing cancellation of a market once trading can occur
...
3.7 WHEN a rebalance is scheduled and executed on a live Solo AMM market THEN the system SHALL CONTINUE TO apply reserve and baseline deltas correctly with ACI updates
