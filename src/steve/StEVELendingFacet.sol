...
contract StEVELendingFacet is StEVELendingLogic, ReentrancyGuardModifiers {
    ...
    function borrow(
        ...
        uint256 collateralUnits,
        uint40 duration
    ) external payable nonReentrant basketExists(LibStEVEStorage.PRODUCT_ID) returns (uint256 loanId) {
        ...
        lending.loans[loanId] = LibStEVELendingStorage.Loan({
            borrowerPositionKey: positionKey,
            collateralUnits: collateralUnits,
            ltvBps: LibStEVELendingStorage.DEFAULT_LTV_BPS,
            maturity: maturity
        });
        ...
        emit LoanCreated(
            loanId,
            LibStEVEStorage.PRODUCT_ID,
            positionKey,
            collateralUnits,
            assets,
            principals,
            LibStEVELendingStorage.DEFAULT_LTV_BPS,
            maturity
        );
    }
        ...
        basketExists(LibStEVEStorage.PRODUCT_ID)
    {
        ...
        LibStEVELendingStorage.s().lendingConfig =
            LibStEVELendingStorage.LendingConfig({minDuration: minDuration, maxDuration: maxDuration});
        emit LendingConfigUpdated(
            LibStEVEStorage.PRODUCT_ID, minDuration, maxDuration, LibStEVELendingStorage.DEFAULT_LTV_BPS
        );
    }
    ...
    function configureBorrowFeeTiers(
    ...
    ) external nonReentrant basketExists(LibStEVEStorage.PRODUCT_ID) {
        LibCurrency.assertZeroMsgValue();
        LibAccess.enforceTimelockOrOwnerIfUnset();
        ...
        LibStEVELendingStorage.BorrowFeeTier[] storage tiers = LibStEVELendingStorage.s().borrowFeeTiers;
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            tiers.push(
                LibStEVELendingStorage.BorrowFeeTier({
        ...
        emit BorrowFeeTiersUpdated(LibStEVEStorage.PRODUCT_ID, minCollateralUnits, flatFeeNative);
    }
    ...
    function borrowerLoanCount(uint256 positionId) external view returns (uint256) {
        return LibStEVELendingStorage.s().borrowerLoanIds[LibPositionHelpers.positionKey(positionId)].length;
    }
        ...
        returns (uint256[] memory)
    {
        return _sliceLoanIds(
            LibStEVELendingStorage.s().borrowerLoanIds[LibPositionHelpers.positionKey(positionId)], start, limit
        );
    }
        ...
        returns (BorrowPreview memory preview)
    {
        ...
        preview.productId = LibStEVEStorage.PRODUCT_ID;
    ...
    function previewRepay(uint256 positionId, uint256 loanId) external view returns (RepayPreview memory preview) {
        ...
        (address[] memory assets, uint256[] memory principals) =
            _deriveLoanPrincipals(LibStEVEStorage.s().product, loan.collateralUnits, loan.ltvBps);
        ...
        returns (ExtendPreview memory preview)
    {
        bytes32 positionKey = LibPositionHelpers.positionKey(positionId);
        (uint40 newMaturity, uint256 feeNative) =
            _validateAndQuoteExtension(LibStEVELendingStorage.s(), loanId, addedDuration, positionKey);
    ...
    function getOutstandingPrincipal(address asset) external view returns (uint256) {
        return LibStEVELendingStorage.s().outstandingPrincipal[asset];
    }

    function getLockedCollateralUnits() external view returns (uint256) {
        return LibStEVELendingStorage.s().lockedCollateralUnits;
    }
}
