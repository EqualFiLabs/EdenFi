...
/// @dev Guarded by transientCacheMode for chains lacking TSTORE/TLOAD support.
library TransientSwapCache {
    bool internal constant USE_TSTORE = true;
    bytes32 private constant SLOT_RES_IN = keccak256("equalx.swap.cache.reserveIn");
    bytes32 private constant SLOT_RES_OUT = keccak256("equalx.swap.cache.reserveOut");
    bytes32 private constant SLOT_FEE_POOL = keccak256("equalx.swap.cache.feePoolId");
    ...
    function loadFeePool() internal view returns (uint256 feePoolId) {
        feePoolId = tload(SLOT_FEE_POOL);
    }
}
