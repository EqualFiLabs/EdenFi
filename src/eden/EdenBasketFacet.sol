...
contract EdenBasketFacet is EdenBasketBase, ReentrancyGuardModifiers {
    event BasketCreated(
    ...
    function _nextBasketPoolId(LibAppStorage.AppStorage storage store) private view returns (uint256 pid) {
        pid = store.poolCount;
        if (pid == 0) pid = 1;
